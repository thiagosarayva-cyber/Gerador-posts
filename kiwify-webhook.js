// ═══════════════════════════════════════════════════════
// Netlify Function — Kiwify Webhook
// Recebe pagamento aprovado → gera código → salva Firebase → envia e-mail
// ═══════════════════════════════════════════════════════
...
exports.handler = async (event) => {
  ...
  // Verifica token de segurança do Kiwify
  const token = event.queryStringParameters?.token || event.headers['x-kiwify-token'];
  ...
  if (!aprovado) {
    return { statusCode: 200, body: JSON.stringify({ ok: true, msg: 'Ignorado: ' + status }) };
  }

  // Extrai dados do comprador — formato Kiwify
  ...
  try {
    ...
    await db.collection('clientes').doc(codigo).set({
      ...
      pagamento: 'Kiwify',
      empresa:   '',
      obs:       `Gerado automaticamente via Kiwify.`,
      dias,
      bloqueado: false,
